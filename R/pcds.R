...
#'
#' \code{pcds} is a package for construction and visualization of proximity catch digraphs (PCDs)
#' and computation of two graph invariants of the PCDs and testing spatial patterns using these invariants.
...
#'
#' The \code{pcds} package also contains the functions for generating patterns of segregation, association, CSR
...
#' The AS-PCD related tools are provided for 1D and 2D data; PE-PCD related tools are provided for 1D-3D data,
...
#' Contains the auxiliary (or utility) functions for constructing and visualizing Delaunay tessellations in 1D and 2D settings,
#' computing the domination number, constructing the geometrical tools, such as equation of lines for two points,
#' distances between lines and points, checking points inside the triangle etc.,
#' finding the (local) extrema (restricted to Delaunay cells or vertex or edge regions in them).
#'
